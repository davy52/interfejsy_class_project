...
class Window:
    def __init__(self) -> None:
        ...
        self._ax_zbiornik = self._fig_zbiornik.add_subplot(
            xlim=(0, 1),
            ylim=(0, 20)
            )
        ...
        self._canvas_zbiornik = FigureCanvasTkAgg(
            self._fig_zbiornik,
            master=self._zbiornik
            )
        self._canvas_zbiornik.draw()
        self._canvas_zbiornik.get_tk_widget().pack(
            side=tk.BOTTOM,
            fill=tk.BOTH,
            expand=1
            )

        self._label_zbiornik = ttk.Label(
            self._zbiornik,
            text="Poziom zbiornika"
            )
        ...
        self._fig_poziom = plt.figure(
            dpi=100,
            figsize=(5, 3)
            )
        self._ax_poziom = self._fig_poziom.add_subplot(
            xlim=(0, 100),
            ylim=(0, 20)
            )
        self._line_poziom = [0, 0]
        self._line_poziom[0] = self._ax_poziom.plot(
            [],
            [],
            lw=2,
            label="h"
            )
        self._line_poziom[1] = self._ax_poziom.stairs(
            self._zbiornik_traj[0],
            self._zbiornik_traj[1],
            linestyle=":",
            color="green",
            label="wartości zadane",
            linewidth=2
            )
        self._ax_poziom.legend(loc='upper right')
        self._ax_poziom.grid()

        self._canvas_poziom = FigureCanvasTkAgg(
            self._fig_poziom,
            master=self._poziom
            )
        self._canvas_poziom.draw()
        self._canvas_poziom.get_tk_widget().pack(
            side=tk.BOTTOM,
            fill=tk.BOTH,
            expand=1
            )

        self._label_poziom = ttk.Label(
            self._poziom,
            text="Funkcja poziomu w czasie"
            )
        ...
        # Frame symulacja
        self._symulacja = ttk.Frame(
            self._root,
            height=300,
            width=500,
            style='my.TFrame'
            )
        self._symulacja.grid(row=1, column=1)

        self.button = ttk.Button(
            master=self._symulacja,
            text="RUN",
    ...
    def _init_sim(self):
        ...
        self._sim.set_tank_variables(       # charakterystyka zbiornika
            ...
            k=1.2                           # wpółczynnik wypływu cieczy
            )
        self._sim.set_pid_settings(         # nastawy regulatora <IND>
            kp=2,                           # wzmocnienie proporcjonalne
            ki=0.8,                         # wzmocnienie członu całkującego
            kd=0.0,                         # wzmocnienie członu różniczki
            wd=0.1,                         # wmocnienie filtru antiwindup
            deadband=0.0                    # pasmo nieczułości [% y_zad]
            )
        self._sim.set_saturation(           # saturacja urządzenia wykonującego
            sat_low=0,                      # dolna granica
            sat_high=7                      # górna granica
            )
        self._sim.set_initial_condition(    # stan początkowy zbiornika
            h0=0.0
            )
        self._sim.set_sim_time(             # czas symulacji
            t0=0,                           # t0 - czas rozpoczęcia
            t_end=100                       # czas zakończenia symulacji
            )
        self._sim.set_trajectory(           # trajektoria zadana
            y_zadane=[10.0, 5.0, 0.0, 15.0, 5.0],    # wartości zadane
            t_y=[0.0, 20.0, 40.0, 60.0, 80.0]        # czasy zmiany y_zad
            )
    ...
    def run_sim(self):
        ...
        [
            ...
            self._zbiornik_ee,
            self._zbiornik_ff
        ] = self._sim.get_simulation_output()
        self._zbiornik_traj = self._sim.get_trajectory()
